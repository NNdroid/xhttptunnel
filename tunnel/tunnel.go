...
func (st *serverState) Kick(id string) bool {
	...
	if ok {
		...
		v.kicked.Store(true)
		st.lg().Debug("👢 [Server] session kicked by administrator", zap.String("session", id))
		v.Close()
	}
	return ok
}
...
func (st *serverState) KickAll() int {
	...
	if len(victims) > 0 {
		...
		st.lg().Debug("👢 [Server] all sessions kicked by administrator", zap.Int("count", len(victims)))
	}
	return len(victims)
}
